...
THREE.PVControls = function ( object, domElement, scene ) {
	...
	var turnStart = new THREE.Vector2();
	var turnEnd = new THREE.Vector2();
	var turnDelta = new THREE.Vector2();
	...
	var STATE = { NONE : -1, ROTATE : 0, DOLLY : 1, PAN : 2, TOUCH_ROTATE : 3, TOUCH_DOLLY : 4, TOUCH_PAN : 5, TURN : 6 };
	...
	// turn the camera in yaw and pitch, then recompute target
	this.turn = function (dyaw, dpitch) {
	    report("turn yaw: "+dyaw+"  pitch: "+dpitch);
	    //var dx = cam.position.x - target.x;
	    var cam = scope.object;
	    cam.rotation.y += dyaw;
	    cam.rotation.x += dpitch;
	...
	this.pan = function ( deltaX, deltaY ) {
		...
		if ( scope.object instanceof THREE.PerspectiveCamera ) {
			...
			console.warn( 'WARNING: PVControls.js encountered an unknown camera type - pan disabled.' );
	...
	this.dollyIn = function ( dollyScale ) {
		...
		if ( scope.object instanceof THREE.PerspectiveCamera ) {
			...
			console.warn( 'WARNING: PVControls.js encountered an unknown camera type - dolly/zoom disabled.' );
	...
	function onMouseDown( event ) {
		...
		if ( event.shiftKey && event.button === scope.mouseButtons.ORBIT ) {
                            report("start turn...");
			    state = STATE.TURN;
			    turnStart.set( event.clientX, event.clientY );
		}
	...
	function onMouseMove( event ) {
		...
		if ( state === STATE.TURN ) {
			...
			turnEnd.set( event.clientX, event.clientY );
			turnDelta.subVectors( turnEnd, turnStart );

			// rotating across whole screen goes 360 degrees around
			var dyaw = 2 * Math.PI * turnDelta.x / element.clientWidth * scope.turnSpeed ;
			var dpitch = 2 * Math.PI * turnDelta.y / element.clientHeight * scope.turnSpeed ;
			scope.turn(dyaw, dpitch);

			turnStart.copy( turnEnd );
		}
	...
	function onKeyDown( event ) {
	    ...
	    //report("keyDown code: "+event.keyCode);
...
THREE.PVControls.prototype = Object.create( THREE.EventDispatcher.prototype );
THREE.PVControls.prototype.constructor = THREE.PVControls;
